...
6. Scan all markdown files under `sources/**` and `outputs_ai/**` for `<!-- FEEDBACK: ... -->` markers. Collect them with their file path and position.
7. Scan all markdown files under `sources/**` and `outputs_ai/**` for `<!-- AI_SUGGESTION: ... -->` markers. The user flags suggestions via the UI: accepted markers contain `[ACCEPTED]` (e.g., `<!-- AI_SUGGESTION: [ACCEPTED] add a row ... -->`), dismissed markers contain `[DISMISSED]`. Unflagged markers are carried forward. Collect accepted suggestions for execution.
8. Remove `[DISMISSED]` AI_SUGGESTION markers from files (delete the entire HTML comment line).

### Phase 3: Follow Build Scope Directive

9. The build pipeline provides a `BUILD SCOPE` directive in the prompt. Follow it exactly:
...
10. Build a verified inventory of `inputs_human/` using filesystem enumeration. Include all file types: markdown, PDF, DOCX, PPTX, XLSX, CSV, images, and any other documents.
11. For each non-placeholder file:
...
12. Do not ask the user to populate `inputs_human/` if it is empty. An empty `inputs_human/` is normal — the build proceeds with web research alone.
...
13. Read `sources/source_log.md` to see what was fetched and what gaps exist.

14. Read all files in `sources/digests/`. These are compact summaries of each source containing key claims, data points, and relevance assessments. Use these to understand the evidence landscape without loading full article text. Each digest is ~200 words vs. the full source which may be 3,000+ words.

15. **Do not read full source files in `sources/web_research/` at this stage.** You will read specific full sources later in Phase 7 and 8, only when you are actively writing or updating a page that needs detailed evidence from that source.

16. If `sources/source_log.md` shows gaps (Unfetched sources or missing topic coverage), leave an `AI_SUGGESTION` marker in the most relevant output file noting the gap and what sources to try on the next build.
...
1. **Primary data** — government statistics, corporate filings (10-K, annual reports), raw datasets
2. **Government/institutional reports** — regulatory agency publications, central bank reports, IGO data (World Bank, IMF)
3. **Academic papers** — peer-reviewed research, working papers from recognized institutions
4. **Corporate disclosures** — investor presentations, earnings transcripts, press releases
5. **Trade press** — industry-specific publications, consultancy reports (McKinsey, Wood Mackenzie)
6. **General news** — Reuters, Bloomberg, major newspapers
7. **Commentary/blogs** — opinion pieces, analyst commentary, social media

When two sources conflict, favor the higher-ranked source and note the disagreement explicitly.

### Phase 6: Apply Annotations

17. Process each collected FEEDBACK marker:
...
18. Execute each accepted AI_SUGGESTION:
...
19. Determine the wiki structure based on `project.md` topics, the digests from Phase 5, and existing wiki pages (use `_index.md` if this is not a first build).
20. For each wiki topic that needs writing or updating:
...
21. The wiki structure is dynamic. The AI may add, split, merge, or rename pages when doing so improves readability and organization. When restructuring, mention the change in the build log entry.
22. Wiki pages should:
...
23. After writing all wiki pages, generate or update `outputs_ai/wiki/_index.md` with this structure:
...
29. **Source depth check**: verify that source digests in `sources/digests/` contain substantive key claims (not just metadata). Flag any thin digests with an `AI_SUGGESTION` marker recommending re-fetch on the next build.
...
33. Process answered questions collected in Step 4:
...
34. Add any new items to `## Open Questions` in `questions.md` that require user judgment or information the AI cannot infer. Do not duplicate items already present.

### Phase 11: Record and Snapshot

35. Create or update `.build/manifest.json` with:
    ```json
    {
      "version": 1,
      "project_name": "...",
    ...
    ```
36. Prepend a build entry to `change_logs/builds.md` with the build timestamp, scope, what was generated/updated, and any caveats.
37. **Git snapshot:** If the build completed without a fatal execution stop:
    - Run from the project root.
    - `git add -A .`
...
38. On the very first build (no prior manifest), before generating AI content:
