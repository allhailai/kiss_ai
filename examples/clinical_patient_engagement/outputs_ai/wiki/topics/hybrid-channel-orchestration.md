...
Hybrid Channel Orchestration is part of the Clinical Patient Engagement Wiki for Medicaid, dual eligible, MLTSS/HCBS, and virtual care engagement strategy. Read it with [[consent-privacy-compliance]], [[patient-segmentation]], and [[digital-to-live-escalation]] when designing implementation workflows.
...
2. Select a channel based on content sensitivity and friction: SMS for brief nudges, SMS-to-secure-message for PHI-sensitive workflows, AI voice for low-risk synchronous outreach, and live callback for complex cases.
3. Escalate when there is non-response, confusion, risk, distress, repeated failure, caregiver complexity, opt-out, or low AI confidence.
4. Record the outcome, channel used, reason for escalation, and next action in a closed-loop work queue.
