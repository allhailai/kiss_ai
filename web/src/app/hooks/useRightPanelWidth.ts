...
import { readRightPanelWidth, writeRightPanelWidth } from "../rightPanelWidthStorage";
...
function defaultWidthFor(view: View, panelKind: RightPanelKind | null) {
  ...
  return view === "ai" && panelKind ? projectChatDefaultPanelWidth : defaultPanelWidth;
}
...
export function useRightPanelWidth({
  ...
  const [storedWidth, setStoredWidth] = useState(() => readRightPanelWidth());
  const normalizedStoredWidth = useMemo(() => normalizePanelWidth(storedWidth ?? undefined, fallbackWidth), [fallbackWidth, storedWidth]);
  const [dragWidthPx, setDragWidthPx] = useState<number | null>(null);
  const widthPx = clamp(dragWidthPx ?? panelWidthToPx(normalizedStoredWidth), minPanelWidthPx, maxUsablePanelWidthPx());
  const cssValue = dragWidthPx === null ? normalizedStoredWidth : `${Math.round(widthPx)}px`;
  const isResizable = Boolean(panelKind);

  useEffect(() => {
    setDragWidthPx(null);
  }, [normalizedStoredWidth]);
  ...
  const commitWidth = useCallback(
    (nextWidthPx?: number) => {
      ...
      const committedWidth = `${Math.round(committedWidthPx)}px`;
      setDragWidthPx(null);
      writeRightPanelWidth(committedWidth);
      setStoredWidth(committedWidth);
    },
    [dragWidthPx, widthPx],
  );
