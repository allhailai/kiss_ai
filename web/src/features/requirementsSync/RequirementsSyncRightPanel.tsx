        ...
        <div className="agent-edit-proposal-file">
          ...
          {hasDiffs ? (
            proposal.conceptualDiffs.map((diff) => {
              ...
              return (
                <div className="agent-edit-proposal-diff" key={diff.id}>
                  <input
                    aria-label={`${diff.status === "accepted" ? "Reject" : "Accept"} ${diff.title}`}
                    checked={diff.status === "accepted"}
                    disabled={controller.busy}
                    onChange={(event) => controller.updateDiffStatus(proposal.step, diff.id, event.currentTarget.checked ? "accepted" : "rejected")}
                    type="checkbox"
                  />
